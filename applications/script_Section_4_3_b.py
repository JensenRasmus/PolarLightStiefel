#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Script associated to Section 4.1

It can produce the data associated to
 - Figure 3

The script produces .npy files, which 
can be plotted using the Matlab routines 
which can found in the 'figures' folder. 
"""

import numpy as np
import time
import sys
sys.path.append('../resources/')

import Stiefel_Exp_Log     as StEL
import Stiefel_retractions as StRet

    
# set dimensions
n = 10000

    
#for the Euclidean metric: alpha = -0.5
#for the Canonical metric: alpha =  0.0
metric_alpha = -0.5
    
# set number of random experiments
runs = 10
dist = 1.0*np.pi

#initialize
time_array  = np.zeros((8,4))
    # time_array[0,:] : timings full PF 
    # time_array[1,:] : timings full PF inverse
    # time_array[2,:] : timings polar light 
    # time_array[3,:] : timings polar light inverse 
    # time_array[4,:] : timings polar light Cayley
    # time_array[5,:] : timings polar light Cayley inverse 

k = 0
for p in [500,1000,1500,2000]:
#for p in [10,20,30,40,50]:
#for p in [50,100,150,200]:    
    for j in range(runs):
        print('Dim p=', p, 'Run ', j)
        #----------------------------------------------------------------------
        #create random stiefel data
        U0, U1, Xi = StEL.create_random_Stiefel_data(n, p, dist, metric_alpha)
        #----------------------------------------------------------------------
        
        # classical polar factor retraction
        t_start = time.time()      
        U1_pf   = StRet.Stiefel_PF_ret(U0, Xi)
        t_end   = time.time() 
        time_array[0,k] = time_array[0,k] + (t_end-t_start)
        
        t_start = time.time()    
        Xi_pfi  = StRet.Stiefel_PF_inv_ret(U0, U1_pf)
        t_end   = time.time() 
        time_array[1,k] = time_array[1,k] + (t_end-t_start) 
        
        # polar light
        mode = 1 # i.e. "use expm, logm
        t_start = time.time()      
        U1_pl   = StRet.Stiefel_PL_ret(U0, Xi,mode)
        t_end   = time.time() 
        time_array[2,k] = time_array[2,k] + (t_end-t_start)
        
        t_start = time.time()    
        Xi_pli  = StRet.Stiefel_PL_inv_ret(U0, U1_pl,mode)
        t_end   = time.time() 
        time_array[3,k] = time_array[3,k] + (t_end-t_start) 
        
        # polar light plus Cayley
        mode = 2 # i.e. "use Cay, Cay_inv
        t_start = time.time()      
        U1_pl   = StRet.Stiefel_PL_ret(U0, Xi,mode)
        t_end   = time.time() 
        time_array[4,k] = time_array[4,k] + (t_end-t_start)
        
        t_start = time.time()    
        Xi_pli  = StRet.Stiefel_PL_inv_ret(U0, U1_pl,mode)
        t_end   = time.time() 
        time_array[5,k] = time_array[5,k] + (t_end-t_start)   

        # Cayley retraction
        t_start = time.time()      
        U1_cay   = StRet.Stiefel_Cayley(U0, Xi)
        t_end   = time.time() 
        time_array[6,k] = time_array[6,k] + (t_end-t_start)
        
        t_start = time.time()    
        Xi_cay  = StRet.Stiefel_inv_Cayley(U0, U1_cay)
        t_end   = time.time() 
        time_array[7,k] = time_array[7,k] + (t_end-t_start)   

        #sys.exit()
    k = k + 1
    print('Dim:',p, ':time for polar factor     retraction    : ', time_array[0]/runs)
    print('Dim:',p, ':time for polar factor inv retraction    : ', time_array[1]/runs)
    print('Dim:',p, ':time for polar light      retraction    : ', time_array[2]/runs)
    print('Dim:',p, ':time for polar light  inv retraction    : ', time_array[3]/runs)
    print('Dim:',p, ':time for polar light      retraction Cay: ', time_array[4]/runs)
    print('Dim:',p, ':time for polar light  inv retraction Cay: ', time_array[5]/runs)
    print('Dim:',p, ':time for Cayley retraction              : ', time_array[6]/runs)
    print('Dim:',p, ':time for Cayley       invretraction      : ', time_array[7]/runs)

np.save("timings_p_retractions",time_array/runs)
    
# # set dimensions
# n = 10000

    
# #for the Euclidean metric: alpha = -0.5
# #for the Canonical metric: alpha =  0.0
# metric_alpha = -0.5
    
# # set number of random experiments
# runs = 10
# dist = 1.0*np.pi

# #initialize
# time_array  = np.zeros((6,4))
#     # time_array[0,:] : timings full PF 
#     # time_array[1,:] : timings full PF inverse
#     # time_array[2,:] : timings polar light 
#     # time_array[3,:] : timings polar light inverse 
#     # time_array[4,:] : timings polar light Cayley
#     # time_array[5,:] : timings polar light Cayley inverse 

# k = 0
# for p in [500,1000,1500,2000]:
# #for p in [50,100,150,200]:    
#     for j in range(runs):
#         print('Dim p=', p, 'Run ', j)
#         #----------------------------------------------------------------------
#         #create random stiefel data
#         U0, U1, Xi = StEL.create_random_Stiefel_data(n, p, dist, metric_alpha)
#         #----------------------------------------------------------------------
        
#         # classical polar factor retraction
#         t_start = time.time()      
#         U1_pf   = StRet.Stiefel_PF_ret(U0, Xi)
#         t_end   = time.time() 
#         time_array[0,k] = time_array[0,k] + (t_end-t_start)
        
#         t_start = time.time()    
#         Xi_pfi  = StRet.Stiefel_PF_inv_ret(U0, U1_pf)
#         t_end   = time.time() 
#         time_array[1,k] = time_array[1,k] + (t_end-t_start) 
        
#         # polar light
#         mode = 1 # i.e. "use expm, logm
#         t_start = time.time()      
#         U1_pl   = StRet.Stiefel_PL_ret(U0, Xi,mode)
#         t_end   = time.time() 
#         time_array[2,k] = time_array[2,k] + (t_end-t_start)
        
#         t_start = time.time()    
#         Xi_pli  = StRet.Stiefel_PL_inv_ret(U0, U1,mode)
#         t_end   = time.time() 
#         time_array[3,k] = time_array[3,k] + (t_end-t_start) 
        
#         # polar light plus Cayley
#         mode = 2 # i.e. "use Cay, Cay_inv
#         t_start = time.time()      
#         U1_pl   = StRet.Stiefel_PL_ret(U0, Xi,mode)
#         t_end   = time.time() 
#         time_array[4,k] = time_array[4,k] + (t_end-t_start)
        
#         t_start = time.time()    
#         Xi_pli  = StRet.Stiefel_PL_inv_ret(U0, U1,mode)
#         t_end   = time.time() 
#         time_array[5,k] = time_array[5,k] + (t_end-t_start)   
#         #sys.exit()
#     k = k + 1
#     print('Dim:',p, ':time for polar factor     retraction    : ', time_array[0]/runs)
#     print('Dim:',p, ':time for polar factor inv retraction    : ', time_array[1]/runs)
#     print('Dim:',p, ':time for polar light      retraction    : ', time_array[2]/runs)
#     print('Dim:',p, ':time for polar light  inv retraction    : ', time_array[3]/runs)
#     print('Dim:',p, ':time for polar light      retraction Cay: ', time_array[4]/runs)
#     print('Dim:',p, ':time for polar light  inv retraction Cay: ', time_array[5]/runs)

# np.save("timings_p_retractions",time_array/runs)