...
def Stiefel_geodesic_interp_pre(Locs, samples, metric_alpha = 0.0, retra=1):
    ...
    for k in range(len(samples)-1):
        ...
        if retra == 1:
            ...
            # Grassmann-like Quasi geodesics
            Delta = StRe.Stiefel_inv_Quasi_geod(Locs[k,:,:],\
                                             Locs[k+1,:,:],mode = 1)
        elif retra == 5:
            # QR retraction
            Delta = StRe.Stiefel_QR_inv_ref(Locs[k,:,:],\
                                             Locs[k+1,:,:])
        elif retra == 6:
            # Cayley retraction
            Delta = StRe.Stiefel_inv_Cayley(Locs[k,:,:],\
                                             Locs[k+1,:,:])
...
def Stiefel_geodesic_interp(Locs,\
    ...
    if retra ==1:
        ...
        # Grassmann-like Quasi geodesics
        U_star = StRe.Stiefel_Quasi_geod(Locs[pos,:,:], lin_factor*Delta)
    elif retra == 5:
        # Grassmann-like Quasi geodesics
        U_star,R = StRe.Stiefel_QR_ret(Locs[pos,:,:], lin_factor*Delta)
    elif retra == 6:
        # Grassmann-like Quasi geodesics
        U_star = StRe.Stiefel_Cayley(Locs[pos,:,:], lin_factor*Delta)
