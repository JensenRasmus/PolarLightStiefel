...
#
#------------------------------------------------------------------------------
def Stiefel_Cayley(U,Xi):
    # W = Xi - 0.5*U @ (U.T @ Xi)
    # V = np.concatenate((W, -U), axis = 1)
    # Z = np.concatenate((U, W),axis = 1)
    
    # ZTV = Z.T @ V
    # p = ZTV.shape[0]
    # diag_ind = np.diag_indices(p)

    # ZTV = -1.0*t/2 * ZTV
    # ZTV[diag_ind] = 1 + ZTV[diag_ind]

    # ZTU = Z.T @ U
    # MM = np.linalg.solve(ZTV,ZTU)
    # Y = U + t* V @ MM
    A = U.T @ Xi
    B = Xi - U @ A
    ...
    Y = -U + np.linalg.solve(H.T,(B.T+2*U.T)).T
    return Y

#------------------------------------------------------------------------------
# Inverse Cayley retraction
#
#------------------------------------------------------------------------------
def Stiefel_inv_Cayley(U0,U1):
    XTY = U0.T @ U1
    ...
    diag_ind = np.diag_indices(p)
    XTY[diag_ind] = 1 + XTY[diag_ind]
    YTU = XTY.T

    # Idea 1: Multiply with the inverse 
    invXTY = np.linalg.inv(XTY)
    invYTX = invXTY.T

    Xi = 2 * (U1 @ invXTY + U0 @ invYTX - U0)
...
if testeffCay:
    ...
    Y2 = Stiefel_Cayley(U,Xi);

    print(np.linalg.norm(Y2.T@Y2 - np.eye(p)))

    Xi2 = Stiefel_inv_Cayley(U,Y2)

    print(np.linalg.norm(Xi-Xi2))
...
if testCayley:
    ...
    Xi = U0 @ A
    Xi = Xi / np.linalg.norm(Xi,'fro') # Normalize for stability 
    U1 = Stiefel_Cayley(U0,Xi)

    Xi_PL = Stiefel_PL_inv_ret(U0,U1,mode = 2)

    print(np.linalg.norm(U1.T @ U1 - np.eye(p)))
    t1 = time.time()
    Xi2 = Stiefel_inv_Cayley(U0,U1)
    t2 = time.time()

    print(t2 - t1)
    # print(np.linalg.norm(Xi2 - Xi))
